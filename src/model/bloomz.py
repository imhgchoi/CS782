...
class BloomzWrapper(object):
    def __init__(self, args, model_dir, lora_adapter_path=None, memory_for_model_activations_in_gb=2):
        ...
        self.align_target_tokenizer = None
    ...
    def __call__(self, batch, output_log_likelihood=True, output_hidden_states=False, hidden_states_layers_to_output=(-1, -.5), output_only_last_token_hidden_states=False):
        with torch.no_grad():
            ...
            return model_output.hidden_states, logits_before_softmax.cpu(), tokens_log_likelihood, log_likelihood, next_token_logit
    ...
    def generate(self, args, query, max_new_tokens=64, return_logits=True, verbose=False):
        ...
        for score in gen.scores:
            for i, val in enumerate(score.argmax(1)):
                gen_ids[i] = gen_ids[i] + [val]
        return gen_ids
